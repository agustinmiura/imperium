...
jQuery(function($) {
	...
	var applicationDao = {
		getApplications : function(successCb, failureCb) {
			var url = "/webapp/application/list";
	...
	var App = {
		...
		onSearchApplication : function(event) {
			var button = $(event.target);
			var inputField = button.closest("div").find("input");
			var query = inputField.val();

			var page = App.data.pagination.currentPage;
			App.renderGrid({
				page : page
			});
		},
		...
		bindEvents : function() {
			...
			$("#doSearchButton").on("click", me.onSearchApplication);
		},
		/**
		...
		onShowCreateForm : function(event) {
			var url = App.urlManager.getUrlForAppCreate();
		...
		renderGrid : function(parameters) {
			...
			var showLast = (url.indexOf("showLast=1") >= 0);

			var queryField = $("#search");
			...
			if ("page" in parameters) {
				...
				listAppUrl = urlManager.getUrlForAppListWithPage(pageRequested);

				addedParameter = true;
			} else {
				//listAppUrl = '/cors/application';
				listAppUrl = urlManager.getUrlForAppList();
				pageRequested = 0;
			 ...
			 */
			if ((query !== "") && (addedParameter === false)) {
				listAppUrl = listAppUrl + "?query=" + query;
				addedParameter = true;
			} else if ((query !== "") && (addedParameter === true)) {
				listAppUrl = listAppUrl + "&query=" + query;
			}
			/**
			 * Add show last parameter
			 */
			if ((showLast === true) && (addedParameter === true)) {
				listAppUrl = listAppUrl + "&showLast=1";
			} else if ((showLast === true) && (addedParameter === false)) {
				listAppUrl = listAppUrl + "?showLast=1";
			}
			 ...
			 * @todo remove
			 */
			console.log("The list app url is :" + listAppUrl);
			...
			var fetchTemplateCb = function(answer, cb) {
				...
				var options = {
					url : "/imperium/resources/app/templates/Application.List.handlebars",
					cache : false,
					success : successCb
				};
