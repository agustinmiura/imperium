...
var imperium_ui_view_application_EditRoles2 = function() {
	...
	logger.debug("Called edit Roles 2");
	...
	var module = {
		...
		init : function() {
			 ...
			 * @todo remove
			 */
			logger.debug("Called method init");
		...
		getSearchAvailableQuery: function() {
			var input = $("#searchAvailableButton").closest("form").find("input");
			return input.val();
		},
		getSearchSelectionQuery: function() {
			var input = $("#searchSelectionConfig").closest("form").find("input");
			return input.val();
		},
		/**
		...
		onAddAllClicked : function(event) {
			...
			var rawSelection = this.getAllRecordsFromTable(this.data.selector.available);
		...
		onRemoveButton : function(event) {
			...
			var rawSelection = this.getTableSelection(this.data.selector.selection,  "error");
			
			if (rawSelection.length>=1) {
				bootbox.confirm("Are you sure you want to remove the selection", function(result) {
					if (result===true) {
						var selection = me.filterSelection(rawSelection);
		...
		onRemoveAll : function(event) {
			...
			var rawSelection = this.getAllRecordsFromTable(me.data.selector.selection);
			if (rawSelection.length>=1) {
				bootbox.confirm("Are you sure you want to remove the selection", function(result) {
					if (result===true) {
						var selection = me.filterSelection(rawSelection);
						me.selectAllInTableCb(me.data.selector.selection, "error");
						me.doRemoval(selection);
					}
				});
			}
