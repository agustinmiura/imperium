 ...
 */
/**
 * Widget that has two tables one to show the available options to the user and
 * a second table to show the user selection
 * 
 * 
 */
var imperium_ui_widget_ChooserWithoutPagination = function() {
	var me = imperium_ui_widget_ChooserWithoutPagination;
	/**
	 * Instance vars
	 */
	var tableLoaded = {
	...
	var configureFilterSelectionButton = function() {
		...
		var onSearchSelectionClick = function(jqueryEvent) {
			...
			reloadTable(selector + " #userSelection");
	...
	var createAddParameterCbCustomizedForSelection = function(selector, userCb) {
		var cb = function(aoData) {
			var form = $(selector).find("#filterSelectionContainer form");
			var formData = form.serializeArray();
			if (formData.length >= 1) {
				var queryParam = (formData[0]).value;
				if (queryParam !== "") {
	...
	var createSelectedOptionsTableConfigCb = function(selector, config) {
		...
		var addParameterCb = selectionConfig["addParameterCb"];
		var beforeCb = createAddParameterCbCustomizedForSelection(
				me.selector, addParameterCb);
		var columnDefinition = selectionConfig["columnDefinition"];
		var url = selectionConfig["url"];
		var method = selectionConfig["method"];

		var config = {
			"bProcessing" : true,
			"bServerSide" : true,
			"fnServerParams" : beforeCb,
			"sAjaxSource" : url,
			"sServerMethod" : method,
			"sAjaxDataProp" : "data",
			"aoColumns" : columnDefinition,
	...
	var initCb = function() {
		me = imperium_ui_widget_ChooserWithoutPagination;
	};
